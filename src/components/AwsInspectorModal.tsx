...
export const AwsInspectorModal: React.FC<AwsInspectorModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-3xl glass-panel bg-slate-900 border border-slate-700/80 rounded-3xl p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="p-3 rounded-2xl bg-gradient-to-tr from-amazon-orange to-amber-500 text-slate-950 shadow-lg shadow-amazon-orange/20">
            <Layers className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              AWS Cloud Architecture Inspector
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amazon-orange/10 text-amazon-orange border border-amazon-orange/20">
        ...
        <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 mb-6">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
            <div className="w-full md:w-auto px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
              <span className="font-bold text-white block">Next.js Client</span>
              <span className="text-[10px] text-slate-400">AWS Amplify / CloudFront</span>
            </div>
            ...
            <ArrowRight className="w-4 h-4 text-amazon-orange rotate-90 md:rotate-0" />
            <div className="w-full md:w-auto px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-center">
              <span className="font-bold text-white block">AWS Lambda</span>
              <span className="text-[10px] text-slate-400">Matchmaker & Ledger</span>
            </div>
            <ArrowRight className="w-4 h-4 text-amazon-orange rotate-90 md:rotate-0" />
            <div className="w-full md:w-auto px-3.5 py-2.5 rounded-xl bg-gradient-to-r from-amber-950/40 to-orange-950/40 border border-amber-500/40 text-center">
              <span className="font-bold text-amber-300 block">Amazon Bedrock</span>
              <span className="text-[10px] text-amber-400">Claude 3 / Titan</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {awsServices.map((svc) => {
            ...
            return (
              <div
                key={svc.name}
                className="p-4 rounded-xl bg-slate-950/50 border border-slate-800/80 hover:border-slate-700 transition-colors flex items-start gap-3.5"
              >
                <div className={`p-2 rounded-lg bg-gradient-to-br ${svc.color} text-white shrink-0 mt-0.5`}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h5 className="text-sm font-bold text-white">{svc.name}</h5>
                    <span className="text-[11px] font-medium text-amber-400/90">{svc.role}</span>
                  </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
          ...
          <span className="text-slate-500 hidden sm:inline">Serverless Auto-Scaling</span>
        </div>
