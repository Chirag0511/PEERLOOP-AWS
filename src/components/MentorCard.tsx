...
export const MentorCard: React.FC<MentorCardProps> = ({
  ...
  return (
    <div className="bg-slate-900/60 hover:bg-slate-900/90 border border-slate-800/80 hover:border-slate-700 rounded-2xl p-5 transition-all duration-200 flex flex-col justify-between group hover:shadow-lg hover:shadow-slate-950/40">
      
      <div>
        {/* AI Match Badge (if from Bedrock search) */}
        {matchScore && (
          <div className="mb-3.5 flex items-center justify-between text-xs text-amazon-orange bg-amazon-orange/10 border border-amazon-orange/20 px-3 py-1.5 rounded-xl">
            <span className="flex items-center gap-1.5 font-semibold">
              <Sparkles className="w-3.5 h-3.5" />
              {matchScore}% Barter Compatibility
            </span>
            <span className="text-[10px] text-slate-400">AWS Bedrock</span>
          </div>
        )}
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-1">
              ...
              <div className="flex items-center gap-1 text-xs text-amber-400 font-semibold shrink-0">
                ...
                <span className="text-slate-500 text-[11px]">({student.totalSessions})</span>
              </div>
            </div>
        ...
        <div className="mt-4 p-3 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-2 text-xs">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-slate-200 truncate ml-2">
              {primaryOffer}
            </span>
          </div>
          ...
          <div className="flex items-center justify-center -my-1">
            <span className="p-1 rounded-full bg-slate-900 text-slate-500">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-slate-300 truncate ml-2">
              {primarySeeking}
            </span>
          </div>
        </div>
        ...
        {aiMatchReason && (
          <p className="mt-2.5 text-[11px] text-slate-400 italic line-clamp-2">
            "{aiMatchReason}"
          </p>
        )}
      </div>
      ...
      <div className="mt-5 pt-3.5 border-t border-slate-800/80 flex items-center justify-between gap-3">
        ...
        <Link
          ...
          className="px-4 py-2 rounded-xl bg-amazon-orange hover:bg-amber-400 text-slate-950 text-xs font-bold transition-all shadow-sm active:scale-95 flex items-center gap-1.5"
        >
